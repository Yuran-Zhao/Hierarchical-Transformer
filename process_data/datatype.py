...
class Tokens:
    ...
    def __getitem__(self, key):
        if type(key) is str:
            if self.name_to_index.get(key) is None:
                warnings.warn("Unknown token in training dataset: " + key)
    ...
    def sample(self, batch_size, num=5):
        return torch.multinomial(
            self.weights(), num * batch_size, replacement=True
        ).view(batch_size, num)
    ...
    def load(cls, text):
        ...
        for inst in insts:
            for i, arg in enumerate(inst.args):
                if labels.get(arg):
                    inst.args[i] = "CONST"
            for i, fg_arg in enumerate(inst.fg_args):
                for j, pt in enumerate(fg_arg):
                    if labels.get(pt):
                        inst.fg_args[i][j] = "CONST"
        return cls(insts, blocks, meta)

    def tokens(self):
        # return [token for inst in self.insts for token in inst.tokens()]
        return [inst.tokens() for inst in self.insts]

    def fg_tokens(self):
        # return [token for inst in self.insts for token in inst.fg_tokens()]
        return [inst.fg_tokens() for inst in self.insts]
...
class Instruction:
    def __init__(self, op, args, fg_args):
        self.op = op
        self.args = args
    ...
    def load(cls, text):
        cpy_text = text
        text = text.strip()
        if "bnd" == text[:3]:  # get rid of BND prefix
            text = text[:3].strip()
        if "lock" == text[:4]:  # get rid of LOCK prefix
            text = text[:4].strip()
        if text:
            ...
            if args:
                ...
                args = []
            args = ([arg.replace(" ", "") for arg in args] + ["<space>", "<space>"])[:2]
            fg_args = []
            for arg in args:
                tmp_arg = arg
                ...
                fg_arg = []
                if " " in tmp_arg:
                    tmp = tmp_arg.strip().split(" ")
                    fg_arg += tmp[:-1]
                    tmp_arg = tmp[-1]
                if ":" in tmp_arg:
                    tmp = tmp_arg.strip().split(":")
                    assert type(tmp[0]) != list
                    fg_arg.append(tmp[0])
                    fg_arg.append(":")
                    tmp_arg = tmp[1]
                if "[" in tmp_arg:
                    tmp_arg = tmp_arg[1:-1]
                    fg_arg.append("[")
                    fg_arg += pattern_split(tmp_arg)
                    fg_arg.append("]")
                else:
                    fg_arg.append(tmp_arg)
                    assert type(tmp_arg) != list
                # if 'C' in fg_arg:
                #     print("CONST split exists: " + arg)
                #     exit()
                for i in range(len(fg_arg)):
                    if (
                        re.match(r"^0x[0-9a-f]+$", fg_arg[i])
                        and int(fg_arg[i], 16) > 0xFF
                    ):
                        fg_arg[i] = "CONST"
                    if re.match(r"^[0-9]+$", fg_arg[i]) and int(fg_arg[i]) > 0xFF:
                        fg_arg[i] = "CONST"

                fg_args.append(fg_arg)
            return cls(op, args, fg_args)
        else:
            print(cpy_text + " is null")
            exit()
...
def pattern_split(line):
    patterns = [
        r"^([^\*\s+-]+)(\*)([^\*\s+-]+)$",  # 'eax*CONST'
        r"^([^\*\s+-]+)$",  # 'eax' or 'CONST'
        r"^([^\*\s+-]+)(\*)([^\*\s+-]+)([+-])([^\*\s+-]+)$",  # 'eax*CONST + CONST'
        r"^([^\*\s+-]+)([+-])([^\*\s+-]+)(\*)([^\*\s+-]+)$",  # 'eax + eax*CONST'
        r"^([^\*\s+-]+)([+-])([^\*\s+-]+)$",  # 'eax + CONST'
        r"^([^\*\s+-]+)([+-])([^\*\s+-]+)([+-])([^\*\s+-]+)$",  # 'eax + eax + CONST'
        r"^([^\*\s+-]+)([+-])([^\*\s+-]+)(\*)([^\*\s+-]+)([+-])([^\*\s+-]+)$",  # 'eax + eax*CONST + CONST'
    ]
    for i in range(len(patterns)):
        matchobj = re.match(patterns[i], line)
        if matchobj:
            return list(matchobj.groups())
