...
class BottomEmbedding(nn.Module):
    def __init__(
        ...
        self.opcode_embedding_layer = nn.Embedding(
            opcode_size, per_op_dim, padding_idx=padding_idx
        ).to(device)
        self.operand1_embedding_layer = nn.Embedding(
            operand_size, per_op_dim, padding_idx=padding_idx
        ).to(device)
        self.operand2_embedding_layer = nn.Embedding(
            operand_size, per_op_dim, padding_idx=padding_idx
        ).to(device)
